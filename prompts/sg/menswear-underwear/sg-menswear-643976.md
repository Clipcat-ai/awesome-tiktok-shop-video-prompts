---
id: sg-menswear-643976
title: "Men Singlet Shaper Body Shirt Vests Compression Sleeveless Sof — Talking Head (Singapore · Menswear & Underwear)"
market: sg
category: menswear-underwear
video_type: talking-head
duration_sec: 12
hook: pain-point
hooks: [pain-point, result-first]
presenter: real-person
promo: [discount]
voiceover: true
voiceover_lang: en
rank: weekly-1
rank_period: 2026-08-31
source: https://www.tiktok.com/share/video/7642508114230643976
creator: "@kel_tzy87"
published: 2026-05-23
---
# Men Singlet Shaper Body Shirt Vests Compression Sleeveless Sof — Talking Head (Singapore · Menswear & Underwear)
**Weekly #1** · 🇸🇬 Singapore · Menswear & Underwear · week of 2026-08-31

Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7642508114230643976) by [@kel_tzy87](https://www.tiktok.com/@kel_tzy87). Format: **Talking Head** · 12s · hook: **Pain Point** · On-camera host · Discount
**Opening line (from the video):** “Tired of loose fitting active wear? Let me show you something different”

**Pain point it attacks:** Activewear is loose and ill-fitting

**Key visuals:** Residential Building · Palm Tree · Compression Vest · Size Chart · Sidewalk · Fitness Equipment · Anti-slip Mat

## Prompt (English)
```text
[Style]: Realistic product review style, bright and clear visuals, product details are displayed clearly and intuitively, with the natural texture of real outdoor sports life, highlighting the practical wearing advantages of sportswear.
[Environment]: Outdoor residential area in Singapore during daytime, including high-rise white apartment buildings, palm trees and green plants, then switch to a street-side pedestrian path with surrounding shops, greenery and resting crowds, finally arrive at an outdoor fitness area paved with non-slip mats, equipped with blue and yellow public fitness equipment.
[Tone & Pacing]: The overall rhythm is brisk and smooth, progressing at a casual, conversational sharing pace, with tight and natural shot transitions, closely combining product display and actual usage scenarios, maintaining a vibrant and enthusiastic promotional atmosphere throughout.
[Camera]: Use a fixed close-up shot to show the character's outfit effect, then switch to static camera positions for multi-angle product display, use a steady follow shot to capture running dynamics in the sports scene, finally return to a fixed medium shot position in the outdoor fitness area, the entire footage is clear and stable without unnecessary shaking.
[Lighting]: Adequate natural outdoor daylight is used throughout, the light is even, bright and soft, with no strong harsh shadows, which can clearly show the fabric texture of the sports vest and its fit effect on the body, the overall picture is clean and transparent.
[Character]: 28-32 year old Asian male, with healthy wheat-colored smooth and delicate skin, wearing black-rimmed glasses, with smooth facial contours, appearing with no makeup; he has a robust and well-proportioned build, with defined muscle lines on shoulders, back and arms, wearing a full set of sports outfits, sunny, confident and approachable, with relaxed and natural movements, wearing a relaxed and sincere smile throughout.

[Shots]
Shot 1:
  - Duration: 0.0sec - 3.0sec
  - Scene Type: Product design highlight
  - Scene: In front of an outdoor background surrounded by high-rise residential buildings and palm trees, a muscular athletic Asian man wearing black-rimmed glasses stands facing the camera. He grips the hem of the outer black compression sports vest with both hands, gently pulling the vest downward to demonstrate how closely it fits the body. He then leans slightly forward toward the camera and faces the lens to share details about the product. The camera position remains fixed throughout, focusing entirely on the person's upper body for the demonstration.
  - Subject: Tired of loose fitting active wear?  Let me show you something different.  3D compression.
Shot 2:
  - Duration: 3.0sec - 5.5sec
  - Scene Type: Product design highlight
  - Scene: The shot switches to a product display against a solid pure white background, presenting the same black compression sports vest in a three-panel view that showcases the cut and waist breathable texture design from three distinct angles: front, left side, and back. The camera is a fixed still-life display position, with a completely stationary, non-moving perspective.
  - Subject: Design. Supports your core from all angles. Available in all sizes. Perfect for single
Shot 3:
  - Duration: 5.5sec - 6.5sec
  - Scene Type: Full size range fit effect
  - Scene: The frame displays a product size comparison chart that clearly marks the corresponding suitable weight ranges for the five sizes from S to XXL. The layout is neat and clear, with black text on a white background. A fixed camera captures the static chart frame.
Shot 4:
  - Duration: 6.5sec - 9.5sec
  - Scene Type: Product usage scenario
  - Scene: The scene shifts to a street-side pedestrian path at the base of a residential building. The Asian man, wearing the black compression sports vest paired with black athletic shorts, is jogging at a steady pace along the path. His arms swing naturally and his posture is stable, with the vest remaining close to his body without shifting at all throughout the movement. Street-side shops, green plants, and passing pedestrians are visible in the background. A steady tracking shot moves in sync with the man's running motion to capture the upper body movement of the character.
  - Subject: Even during morning runs, it stays in place.  No more adjusting for mid-workout.
Shot 5:
  - Duration: 9.5sec - 13.0sec
  - Scene Type: Order process guide
  - Scene: The scene switches to the outdoor fitness area in the residential district, the Asian man stands next to the fitness equipment, pinches the two side hems of the black compression sports vest with both hands, pulls the vest fully sideways to demonstrate its super elasticity, then releases his hands, the vest immediately springs back to its close-fitting state, finally the man stands naturally facing the camera to finish the sharing. Fixed medium shot position, focusing on the character's movements above the waist throughout.
  - Subject: DM fit to get yours now.  Free delivery in Singapore, limited stock. Thank you.
[Background Sound]:
- No background music or sound effects at all, only clear product narration voice is retained. There are no additional musical elements throughout, forming a pure voiceover audio track with no music and heavy copy focus, possibly accompanied by extremely faint natural ambient sounds, breathing sounds or clothing rustle.
[Transition / Editing]:
- Adopt natural and smooth scene jump editing, transition seamlessly from the character's real-scene demonstration to product still life display, size chart screen, then smoothly connect to the outdoor running dynamic scene, finally return to the product detail display in the outdoor fitness area, the whole process has a tight rhythm and natural connection, fitting the visual rhythm of short video product promotion.
```
## 提示词（中文）
```text
[Style]: 写实产品测评风，画面明亮通透，产品细节展示清晰直观，带有真实户外运动生活的自然质感，突出运动服饰的实穿优势。
[Environment]: 白天新加坡户外居民区，含高层白色居民楼、棕榈树绿植，后续切换至临街步道，周边有商铺、绿植与休憩人群，最后到铺有防滑地垫的户外健身区，摆放蓝黄配色公共健身器材。
[Tone & Pacing]: 整体节奏明快流畅，以生活化口语化的分享节奏推进，镜头衔接紧凑自然，产品展示与实际使用场景结合紧密，全程保持活力热情的推广氛围。
[Camera]: 采用固定近景展示人物穿搭效果，后续切换产品多角度静物展示机位，运动场景使用稳定跟拍捕捉跑步动态，最后回到户外健身区固定中景机位，全程画面清晰稳定无多余晃动。
[Lighting]: 全程使用白天充足的自然户外日光，光线均匀明亮柔和，没有强烈的明暗阴影，能够清晰展示运动背心的面料质感与上身的贴合效果，整体画面干净通透。
[Character]: 28-32岁亚裔男性，小麦色健康肌肤，肤质光滑细腻，佩戴黑框眼镜，脸型线条流畅，纯素颜出镜；身材健硕匀称，肩背与手臂肌肉线条明显，身着全套运动穿搭，气质阳光自信有亲和力，动作舒展自然，全程面带轻松真诚的笑容。

[Shots]
Shot 1:
  - 时长: 0.0sec - 3.0sec
  - 镜头类型: 产品设计亮点
  - 画面: 在高层居民楼与棕榈树环绕的户外背景前，佩戴黑框眼镜的亚裔健硕运动男士站在镜头前方，双手捏住外层黑色压缩运动背心的下摆，将背心微微向下拉扯，展示背心贴身贴合的效果，随后身体微微前倾靠近镜头，面向镜头进行产品分享。机位保持固定，全程聚焦人物上半身展示。
  - 口播: Tired of loose fitting active wear?  Let me show you something different.  3D compression.
Shot 2:
  - 时长: 3.0sec - 5.5sec
  - 镜头类型: 产品设计亮点
  - 画面: 画面切换为白色纯色背景的产品展示图，以三联视图呈现同一件黑色压缩运动背心，分别展示背心正面、左侧侧面、背面三个不同角度的版型与腰部透气纹理设计。固定静物展示机位，视角静止无移动。
  - 口播: Design. Supports your core from all angles. Available in all sizes. Perfect for single
Shot 3:
  - 时长: 5.5sec - 6.5sec
  - 镜头类型: 全尺码适配效果
  - 画面: 画面展示产品尺码对照表，表格内清晰标注S到XXL五个尺码对应的适配体重范围，白底黑字排版清晰规整。固定机位拍摄静态表格画面。
Shot 4:
  - 时长: 6.5sec - 9.5sec
  - 镜头类型: 产品使用场景
  - 画面: 场景切换到居民楼下的临街步道，亚裔男士穿着黑色压缩运动背心搭配黑色运动短裤，正在步道上匀速慢跑，双臂自然摆动，身体姿态平稳，全程保持背心贴身不位移的状态，背景中可以看到街边商铺、绿植与路过的行人。稳定跟拍镜头，跟随人物跑步动作同步移动，捕捉人物上半身运动状态。
  - 口播: Even during morning runs, it stays in place.  No more adjusting for mid-workout.
Shot 5:
  - 时长: 9.5sec - 13.0sec
  - 镜头类型: 下单流程指引
  - 画面: 场景切换到居民区户外健身区域，亚裔男士站在健身器材旁，双手捏住黑色压缩运动背心的两侧下摆，将背心横向充分拉扯开来，展示背心的超强弹力，随后松开手，背心立刻回弹恢复贴身状态，最后男士自然站立面向镜头完成分享。固定中景机位，全程聚焦人物腰部以上的动作展示。
  - 口播: DM fit to get yours now.  Free delivery in Singapore, limited stock. Thank you.
[Background Sound]:
- 无任何配乐或背景音效，仅保留清晰的产品讲解人声，全程无额外音乐元素，为无音乐+强文案风格的纯口播音轨，仅可能伴随极轻微的自然环境音、呼吸声或衣物摩擦声。
[Transition / Editing]:
- 采用自然顺畅的场景跳转剪辑，从人物实景展示平滑过渡到产品静物展示、尺码表画面，再无缝衔接户外跑步动态场景，最后回到户外健身区的产品细节展示，全程节奏紧凑衔接自然，符合短视频产品推广的视觉节奏。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [sg-menswear-643976](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/sg-menswear-643976?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=sg-menswear-643976)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[sg-menswear-643976](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/sg-menswear-643976?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=sg-menswear-643976)
🤖 **Driving an AI agent?** `clipcat prompt search "Menswear & Underwear"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
