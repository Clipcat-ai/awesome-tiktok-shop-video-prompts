---
id: es-menswear-506903
title: "Men's Solid-Color Durable… — OOTD Showcase (Spain · Menswear & Underwear)"
market: es
category: menswear-underwear
video_type: ootd
duration_sec: 8
hook: pov-scenario
hooks: [pov-scenario]
presenter: real-person
promo: [none]
voiceover: true
voiceover_lang: en
rank: weekly-1
rank_period: 2026-08-31
source: https://www.tiktok.com/share/video/7678940729380506903
creator: "@nysportswear9"
published: 2026-08-31
---
# Men's Solid-Color Durable… — OOTD Showcase (Spain · Menswear & Underwear)
**Weekly #1** · 🇪🇸 Spain · Menswear & Underwear · week of 2026-08-31
Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7678940729380506903) by [@nysportswear9](https://www.tiktok.com/@nysportswear9). Format: **OOTD Showcase** · 8s · hook: **POV Scenario** · On-camera host · No promo
**Opening line (quoted from the video):** “Come on, Michael Boguto”

**Pain point it attacks:** Work pants lack elasticity and restrict movement

**Key visuals:** Rail Track Walk · Work Pants · White Shirt · Outdoor Shoes · Wristwatch · Urban Skyscraper · Light Rail Train

## Prompt (English)
```text
[Style]: Hard-core outdoor workwear aesthetic, sharp and crisp image quality, moderate color saturation that emphasizes the texture of the pants, highlighting a tough, capable masculine demeanor.
[Environment]: On the paved walkway between the rails of an urban light rail platform on a clear sunny day, clean stone slab walkway set between two rust-colored rails, with the skyline of modern city high-rises and a parked light rail train visible in the distance, bright natural light flooding the entire scene for an open, unobstructed atmosphere.
[Tone & Pacing]: Short-form video product showcase style, snappy and sharp pacing, every outfit demonstration movement lands precisely on the music's drum beats, delivering strong visual impact to highlight the durability and superior cut of the work pants.
[Camera]: Static medium shot framing used throughout the entire sequence, precisely capturing the subject's lower body from the waist to the ankles, no unnecessary camera shake, fully focused on showcasing the pants' silhouette and dynamic details.
[Lighting]: Abundant daytime natural light falls evenly across the scene, bright and soft, clearly defining the fabric texture and three-dimensional tailoring structure of the work pants with no harsh, sharp shadows.
[Character]: East Asian male, mature rugged young adult aged 28-35, healthy wheat-toned skin, defined arm muscle lines with firm skin, sharp jawline and clean facial contours, no makeup, natural look, well-developed proportional legs with smooth muscle lines, wearing a white shirt with rolled-up sleeves paired with a workwear belt, outdoor sneakers, and a wristwatch as a subtle accent, presenting a capable outdoor commuter workwear style, steady and sharp temperament, movements are deliberate and focused on demonstrating the pants' features, calm neutral facial expression with no unnecessary gestures.

[Shots]
Shot 1:
  - Duration: 0.0sec - 3.5sec
  - Scene Type: Multi-style selection showcase
  - Scene: A muscular man stands on the rail walkway, wearing a white shirt with rolled-up sleeves and a khaki workwear belt, paired with dark grey multi-pocket work pants and grey outdoor shoes. He first pulls both pant legs outward simultaneously with both hands to demonstrate the fabric's stretch and relaxed silhouette, then turns to a side profile, lifts one leg in a striding motion, and smoothly slips his hand into the side pocket of the work pants, all movements fluid and natural, with the open light rail platform and distant city skyline in the background.
  - Subject: Come on, Michael Boguto.
Shot 2:
  - Duration: 3.5sec - 6.0sec
  - Scene Type: Multi-style selection showcase
  - Scene: Scene shifts, the man changes into the identical multi-pocket work pants in solid black, still standing on the same section of rail walkway. He stands with legs apart in a half-squat position, rests both hands naturally on his thighs, and performs repeated small controlled squats in time with the music rhythm to demonstrate the full range of unrestricted movement the work pants allow even during large motions, with the parked light rail train visible in the far background.
  - Subject: I'm a nigga with a attitude, thanks nigga.
Shot 3:
  - Duration: 6.0sec - 9.0sec
  - Scene Type: Multi-style selection showcase
  - Scene: The man changes into the identical multi-pocket work pants in army green, returning to the center of the rail walkway. He once again grabs the outer sides of the left and right pant legs with each respective hand, pulls outward with force to clearly demonstrate the army green work pants' fabric elasticity and silhouette tolerance, with his lower body kept as the core focus of the frame throughout, for a perfectly stable and clear shot.
  - Subject: Yo, now don't give a fuck, I keep the gangsta, yo.
[Background Sound]:
- Highly energetic hip-hop/rap background track with prominent bass drum beats and a strong rhythmic groove, styled as street/urban hip-hop, fast-paced, loud, full of attitude and edge, perfectly matching the vibe of hard-core workwear apparel showcase.
[Transition / Editing]:
- Hard cut editing transitions, each time the scene jumps directly to the next different color work pant design, synced exactly to the heavy drum beat hits of the background music for crisp, snappy pacing that quickly completes the showcase of three different colored work pants.
```
## 提示词（中文）
```text
[Style]: 硬核户外工装风，画面质感锐利清晰，色彩饱和度适中突出裤子质感，凸显硬朗干练的男性气质。
[Environment]: 晴朗白天的城市轨道交通站台铁轨步道上，干净的石板步道嵌在两条锈色铁轨之间，远处可见现代城市高楼轮廓与停靠的轻轨列车，明亮的自然光铺满整个场景，环境开阔通透。
[Tone & Pacing]: 短视频产品展示风格，节奏明快利落，每个穿搭展示动作都精准踩中音乐鼓点，视觉冲击力强，突出工装裤的耐用性与版型优势。
[Camera]: 全程采用中景固定机位拍摄，精准取景人物从腰部到脚踝的下半身区域，无多余镜头晃动，聚焦展示裤子的版型与动态细节。
[Lighting]: 充足的日间自然光均匀洒落，光线明亮柔和，清晰勾勒出工装裤的面料纹理与立体剪裁结构，无明显生硬阴影。
[Character]: 亚洲男性，28-35岁成熟硬朗中青年，小麦色健康肤质，手臂肌肉线条明显皮肤紧实，下颌线清晰面部线条利落，无妆容原生状态，健硕匀称腿部肌肉线条流畅，身着挽袖白衬衫搭配工装腰带、户外运动鞋，戴腕表点缀，风格干练户外通勤工装，气质沉稳利落，动作舒展专注展示裤子特性，面部沉稳无多余表情。

[Shots]
Shot 1:
  - 时长: 0.0sec - 3.5sec
  - 镜头类型: 展示多款式选择
  - 画面: 健硕的男性站在铁轨步道上，上身穿着挽起袖子的白色衬衫，搭配卡其色工装腰带，下身着深灰色多口袋工装裤，脚踩灰色户外鞋。他先是双手同时向两侧拉扯裤腿，展示裤子面料的弹力与宽松版型，随后转身侧身，抬起一条腿做出跨步动作，顺势将手插入工装裤的侧袋中，动作流畅自然，背景是开阔的轨道交通站台与远处城市轮廓。
  - 口播: Come on, Michael Boguto.
Shot 2:
  - 时长: 3.5sec - 6.0sec
  - 镜头类型: 展示多款式选择
  - 画面: 场景切换，男性换上纯黑色同款多口袋工装裤，依旧站在同一段铁轨步道上。他双腿分开呈半蹲姿态，双手自然搭在大腿处，跟随音乐节奏反复小幅蹲起，展示工装裤在大幅度动作下完全不紧绷的活动空间，背景远处可以看到停靠的轻轨列车。
  - 口播: I'm a nigga with a attitude, thanks nigga.
Shot 3:
  - 时长: 6.0sec - 9.0sec
  - 镜头类型: 展示多款式选择
  - 画面: 男性换上军绿色同款多口袋工装裤，站回铁轨步道中间位置。他再次双手分别抓住左右裤腿的外侧位置，向两侧发力拉扯，清晰展示军绿色工装裤的面料弹力与版型包容度，全程下半身处于画面核心区域，画面稳定清晰。
  - 口播: Yo, now don't give a fuck, I keep the gangsta, yo.
[Background Sound]:
- 非常强劲的嘻哈/说唱背景音乐，带有明显低音鼓点和节奏感强烈的节拍，风格为街头/黑人嘻哈风，节奏快、音量大、充满攻击性和态度感，适配硬核工装服饰展示的氛围。
[Transition / Editing]:
- 采用硬切转场的剪辑方式，每切换一款不同颜色的工装裤画面就直接跳转，搭配背景音乐的强鼓点节点切换镜头，节奏干脆利落，快速完成三款不同颜色工装裤的展示。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [es-menswear-506903](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/es-menswear-506903?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=es-menswear-506903)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[es-menswear-506903](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/es-menswear-506903?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=es-menswear-506903)
🤖 **Driving an AI agent?** `clipcat prompt search --query "Menswear & Underwear ootd showcase"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
