---
id: vn-luggage-793672
title: "Balo nam nữ SPORTBAG chống nước… — Handheld Demo (Vietnam · Luggage & Bags)"
market: vn
category: luggage-bags
video_type: handheld-demo
duration_sec: 16
hook: pov-scenario
hooks: [pov-scenario]
presenter: real-person
promo: [none]
voiceover: true
voiceover_lang: vi
rank: weekly-1
rank_period: 2026-08-17
source: https://www.tiktok.com/share/video/7641993127531793672
creator: "@tintinshophcm"
published: 2026-05-20
---
# Balo nam nữ SPORTBAG chống nước… — Handheld Demo (Vietnam · Luggage & Bags)
**Weekly #1** · 🇻🇳 Vietnam · Luggage & Bags · week of 2026-08-17
Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7641993127531793672) by [@tintinshophcm](https://www.tiktok.com/@tintinshophcm). Format: **Handheld Demo** · 16s · hook: **POV Scenario** · On-camera host · No promo
**Opening line (translated from the video):** “Guys who need a nice, durable and compact backpack don't miss this one”

**Pain point it attacks:** No specific pain point mentioned

**Key visuals:** Rolling Shutter Wall · Black Backpack · Orange Zipper · Notebook Computer · Thin Books · Folded Clothes · White Mask · Gray Hoodie

## Prompt (English)
```text
[Style]: A realistic e-commerce short video showcasing quality goods, with bright and clear imagery that highlights the detailed texture and practical features of the backpack, matching the visual style of product recommendation and sales content on short video platforms.
[Environment]: Next to a roller shutter wall outdoors during the daytime, the background is a light beige horizontal louvered metal roller shutter with sufficient and even lighting. The entire scene is simple, clean, and free of unnecessary clutter that would cause distraction.
[Tone & Pacing]: The overall atmosphere is enthusiastic and approachable, a typical short video UGC sales style. The rhythm is tight and smooth, every demonstration action connects naturally, quickly presenting the backpack's appearance, capacity, and detailed features one by one to grab the audience's attention rapidly.
[Camera]: Primarily uses medium-full shots showing the person and close-up detail shots of the product, paired with slight forward camera movement. The camera stays stable throughout, with the focus always accurately locked on different display areas of the backpack to clearly present every product detail.
[Lighting]: Uses soft, even daytime natural light that is bright and sufficient, with no harsh shadows. It clearly showcases the texture of the backpack's black fabric and the orange decorative details, resulting in an overall clean, transparent image with accurate color reproduction.
[Character]: East Asian male around 20-25 years old, with fresh, clean skin and distinct, neat facial features. He wears a white medical face mask with no additional makeup, has a slender, well-proportioned build, and wears a gray and white color-block raglan sleeve sweatshirt paired with light blue jeans. He has a youthful, friendly demeanor, moves naturally and smoothly to guide the audience to focus on the backpack details, and is full of enthusiastic energy.

[Shots]
Shot 1:
  - Duration: 0.0sec - 4.0sec
  - Scene Type: Handheld product demonstration
  - Scene: The young man supports the bottom of the backpack with one hand, and points to the brand logo on the front of the backpack with his other hand. Then he points to the front of the backpack and the top zipper with each hand respectively, demonstrating the overall black appearance of the backpack, as well as the orange zipper and line decoration details, against the light beige roller shutter wall background.
  - Subject: Anh em nào đang cần một chiếc ba lô vừa đẹp vừa bền nhưng nhỏ gọn
Shot 2:
  - Duration: 4.0sec - 5.5sec
  - Scene Type: Product versatility demonstration
  - Scene: The camera cuts to a close-up of the backpack's front pocket being opened. The young man pulls open the front pocket zipper of the backpack with both hands, revealing a laptop and several thin books placed inside the pocket. Then the young man brings the backpack back to his side, and points to the text 'SPORTBAG' printed on the front of the backpack.
  - Subject: thì đừng bỏ qua mẫu này nhé. Thiết kế trẻ trung đứng cực sang, đựng vừa lắp,
Shot 3:
  - Duration: 5.5sec - 8.0sec
  - Scene Type: Product usage scenario demonstration
  - Scene: The young man holds the top handle of the backpack with one hand, fully opening the main compartment of the backpack towards the camera to clearly show the large-capacity space inside the main compartment, where a laptop, multiple books, and folded clothing are neatly placed, intuitively demonstrating the backpack's loading capacity.
Shot 4:
  - Duration: 8.0sec - 12.5sec
  - Scene Type: Ease of use demonstration
  - Scene: The camera returns to a medium shot of the young man. He sequentially pulls open the two layers of zippers on the top of the backpack with both hands to demonstrate the smooth sliding feel of the zippers, then zips them back closed. He gently brushes the palm of his hand over the surface of the backpack fabric to show the thick, sturdy texture of the material.
Shot 5:
  - Duration: 12.5sec - 16.0sec
  - Scene Type: Order process guidance
  - Scene: The young man turns the backpack sideways to the camera, points to the side pocket of the backpack, and looks at the camera with a bright, cheerful smile. Finally, the young man gently holds the backpack and adjusts his demonstration posture.
  - Subject: Bạn bấm vào liên kết sản phẩm ở góc dưới bên trái để xem thêm nhé Ngay kéo hết
[Background Sound]:
- No background music throughout, this is a pure live human voice sales presentation format with no background sound effects or musical accompaniment, only the host's explanatory voice is retained. The pace is tight and the speaking speed is fast, making the audience's attention fully focused on the product description and promotional information.
[Transition / Editing]:
- Uses coherent, smooth action-connected editing, naturally switching between medium shots of the person and close-ups of the product with no abrupt jumps. It maintains consistent demonstration rhythm throughout, fitting the product recommendation logic of sales videos.
```
## 提示词（中文）
```text
[Style]: 写实风电商好物展示短视频，画面明亮通透，突出背包的细节质感与实用属性，符合短视频平台种草带货的视觉风格。
[Environment]: 白天户外的卷帘墙面旁，背景是浅米色横向百叶状的金属卷帘，光线充足均匀，整体场景简洁干净，无多余杂物干扰。
[Tone & Pacing]: 整体氛围热情接地气，是典型的短视频UGC带货风格，节奏紧凑流畅，每一个展示动作衔接自然，快速将背包的外观、容量、细节特点逐一呈现，快速抓住观众注意力。
[Camera]: 以中全景人物展示、产品特写细节镜头为主，搭配轻微的镜头推进运镜，全程保持稳定，画面焦点始终精准落在背包的不同展示区域，清晰呈现每一处产品细节。
[Lighting]: 采用柔和均匀的日间自然光，光线明亮充足，没有生硬的阴影，将背包的黑色面料质感与橙色装饰细节清晰展现，整体画面干净通透，色彩还原准确。
[Character]: 20-25岁左右东亚男性，皮肤清爽干净，五官立体清爽，佩戴白色医用口罩无额外妆容，身形匀称偏瘦，身穿灰白拼色插肩袖卫衣搭配浅蓝色牛仔裤，气质青春亲切，动作自然流畅引导观众关注背包细节，情绪饱满热情。

[Shots]
Shot 1:
  - 时长: 0.0sec - 4.0sec
  - 镜头类型: 手持商品展示
  - 画面: 男生单手托住背包底部，另一只手指向背包正面的品牌标识位置，随后双手分别指向背包的正面与顶部拉链，展示背包的整体黑色外观、橙色拉链与线条装饰细节，背景是浅米色卷帘墙面。
  - 口播: Anh em nào đang cần một chiếc ba lô vừa đẹp vừa bền nhưng nhỏ gọn
Shot 2:
  - 时长: 4.0sec - 5.5sec
  - 镜头类型: 产品多功能性
  - 画面: 镜头切换到背包前袋的打开特写，男生双手拉开背包正面的前袋拉链，可以看到袋内放置着笔记本电脑与几本薄本书籍，随后男生将背包收回身侧，用手指向背包正面印有的“SPORTBAG”字样。
  - 口播: thì đừng bỏ qua mẫu này nhé. Thiết kế trẻ trung đứng cực sang, đựng vừa lắp,
Shot 3:
  - 时长: 5.5sec - 8.0sec
  - 镜头类型: 产品使用场景
  - 画面: 男生单手拎住背包顶部的手提带，将背包主舱完全打开面向镜头，清晰展示主舱内的大容量空间，里面妥善放置着笔记本电脑、多本书籍以及折叠好的衣物，直观呈现背包的装载能力。
Shot 4:
  - 时长: 8.0sec - 12.5sec
  - 镜头类型: 展示易用性
  - 画面: 镜头回到男生的中景画面，男生用双手依次拉开背包顶部的两层拉链，展示拉链的顺滑使用感，随后将拉链拉合复位，手掌轻轻拂过背包的面料表面，展示面料的厚实质感。
Shot 5:
  - 时长: 12.5sec - 16.0sec
  - 镜头类型: 下单流程指引
  - 画面: 男生将背包侧对镜头，手指指向背包的侧袋位置，面带灿烂的笑容看向镜头，最后男生轻轻扶住背包，调整展示姿态。
  - 口播: Bạn bấm vào liên kết sản phẩm ở góc dưới bên trái để xem thêm nhé Ngay kéo hết
[Background Sound]:
- 全程无背景音乐，为纯人声口播带货形式，无背景音效或音乐衬托，仅保留主播的讲解声音，节奏紧凑、语速较快，让观众注意力完全集中在产品描述和促销信息上。
[Transition / Editing]:
- 采用连贯流畅的动作衔接剪辑，通过镜头在人物中景与产品特写之间的自然切换，无生硬跳转，全程保持展示节奏的连贯性，贴合带货视频的种草逻辑。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [vn-luggage-793672](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/vn-luggage-793672?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=vn-luggage-793672)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[vn-luggage-793672](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/vn-luggage-793672?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=vn-luggage-793672)
🤖 **Driving an AI agent?** `clipcat prompt search --query "Luggage & Bags handheld demo"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
