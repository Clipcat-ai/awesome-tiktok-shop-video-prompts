---
id: my-household-955797
title: "Hulmers - Adjustable Cooling Fan 6… — Lifestyle Scene (Malaysia · Household Appliances)"
market: my
category: household-appliances
video_type: lifestyle-scene
duration_sec: 11
hook: pov-scenario
hooks: [pov-scenario]
presenter: hands-only
promo: [none]
voiceover: false
voiceover_lang: null
rank: weekly-1
rank_period: 2026-08-24
source: https://www.tiktok.com/share/video/7658845296750955797
creator: "@ceritabarang"
published: 2026-07-05
---
# Hulmers - Adjustable Cooling Fan 6… — Lifestyle Scene (Malaysia · Household Appliances)
**Weekly #1** · 🇲🇾 Malaysia · Household Appliances · week of 2026-08-24
Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7658845296750955797) by [@ceritabarang](https://www.tiktok.com/@ceritabarang). Format: **Lifestyle Scene** · 11s · hook: **POV Scenario** · Hands only · No promo
**Opening beat:** “In warm home desk scene, hands open brand carton and take out white desk fan”

**Key visuals:** Wooden Desk · Brand Carton · White Desk Fan · Adjustment Knob · Purple Ambient Light · Tissue · Coffee Cup · Breakfast Plate

## Prompt (English)
```text
[Style]: Exquisite realistic home appliance product showcase style, with delicate and translucent image rendering, rich layers of light and shadow, focusing on restoring the fan's real texture and the cozy atmosphere of its usage scenarios, the overall visual is clean and premium.
[Environment]: The opening is set on a home desktop bathed in warm natural light, with a wooden tabletop paired with a storage bookshelf and green plants in the background; then it switches sequentially to a low-light atmospheric home environment, a daily living room with soft lighting, a home office desktop, a sunlit breakfast table by the window, and finally returns to the low-light atmospheric desktop scene, creating a comfortable and relaxing home usage atmosphere throughout.
[Tone & Pacing]: The overall rhythm is slow and smooth, following the product demonstration logic of unboxing → function showcase → multi-scenario adaptation, presenting the product's core selling points clearly step by step, with a relaxing and soothing viewing experience.
[Camera]: Centered on the product, multi-angle camera movements are used: the opening uses a fixed medium shot to show the unboxing action, then switches to a macro close-up lens to focus on the fan's adjustment knob, later uses a panoramic shot to show the fan's operating status in different scenarios, the camera moves steadily and smoothly, always clearly highlighting product details and operating effects.
[Lighting]: The opening uses warm soft natural light spreading over the tabletop to create a cozy unboxing atmosphere; the low-light scenes use the fan's built-in purple ambient light as the core visual light source, paired with ambient fill light to outline the product's outline; daytime scenes use translucent natural light for even illumination, clearly showing the pure white body texture and air blowing effect of the fan.
[Character]: Only the person's hands are shown, belonging to a 20-30 year old individual, with delicate hand skin, neatly trimmed nails with a natural translucent light sheen, no extra decorations on the hands, gentle and precise movements, conveying the anticipation and delicacy when unboxing a nice item, no facial, full-body or outfit shots of the person are included.

[Shots]
Shot 1:
  - Duration: 0.0sec - 1.5sec
  - Scene Type: Unboxing
  - Scene: In the warm-lit home wooden desktop scene, the 'Hulrers' brand cardboard box printed with the fan product pattern is placed in the center of the frame. A pair of hands extends from both sides of the frame, fingers gently pinch the flip lid of the cardboard box and unfold it, then steadily take the white desktop fan wrapped in transparent plastic sealing film out of the cardboard box and place it on the tabletop, completing the unboxing action.
Shot 2:
  - Duration: 1.5sec - 3.0sec
  - Scene Type: Demonstrate product ease of use
  - Scene: The camera switches to a macro close-up perspective, focusing on the adjustment knob on the side of the white fan body. The fingertip of one hand extends out, gently pinches the knob and turns it slowly, the movement is soft and precise.
Shot 3:
  - Duration: 3.0sec - 5.5sec
  - Scene Type: Product usage scenario
  - Scene: The scene switches to a low-light home environment, the fan runs steadily on the tabletop, soft purple ambient light seeps out from the fan ring and the base, the light outlines the fan's rounded silhouette in the dark space, the brand logo at the center of the body is clearly visible.
Shot 4:
  - Duration: 5.5sec - 7.5sec
  - Scene Type: Demonstrate product effectiveness
  - Scene: The scene switches to a daily living room desktop bathed in natural light, the white fan runs facing the camera, the strong airflow keeps blowing the tissue placed on the tabletop next to the fan to flutter continuously, the frame clearly shows the fan's wind power effect, white mugs are placed on both sides of the tabletop, the background is a wall decorated with paintings and lush green plants.
Shot 5:
  - Duration: 7.5sec - 8.5sec
  - Scene Type: Product design highlights
  - Scene: The scene switches to a home office desktop, a laptop and a wireless mouse are placed on the tabletop. One hand grips the hidden handle on the top of the fan, easily lifting the entire unit off the tabletop, demonstrating the product's portable design for easy carrying.
Shot 6:
  - Duration: 8.5sec - 10.0sec
  - Scene Type: Set usage scenario
  - Scene: The scene switches to a sunlit breakfast table by the window, the tabletop is covered with an orange and white checkered tablecloth, with a coffee cup and a plate holding croissants and grapes placed on it. The white fan is placed on the tablecloth and runs steadily, the natural green plants and light outside the window create a pleasant breakfast atmosphere.
Shot 7:
  - Duration: 10.0sec - 11.5sec
  - Scene Type: Scene-based lifestyle showcase
  - Scene: Return to the low-light home desktop scene, the fan continues to run steadily, the purple ambient light and the reflection on the tabletop echo each other, the laptop next to it is turned on, the frame ends with a freeze frame on the complete product image of the fan.
[Background Sound]:
- No melodic background music, composed entirely of a sequence of operation and ambient sound effects: including the tearing sound of unboxing, the soft thud of items being placed, the startup chime of an electronic device, the click of a rotating knob, and the continuous low-frequency hum of a fan motor starting and running. The overall audio track is clean and crisp, highlighting the product's operation feedback and running texture, with a listening experience full of tech vibes and unboxing immersion.
[Transition / Editing]:
- Smooth scene transition editing is adopted, connecting different usage environments through natural scene switches, the overall transitions are seamless and unbroken, fully linking the entire process demonstration of product unboxing, function demonstration and multi-scenario adaptation.
```
## 提示词（中文）
```text
[Style]: 精致写实的家电产品展示风，画面渲染细腻通透，光影层次丰富，侧重还原风扇的真实质感与使用场景的舒适氛围，整体视觉干净高级。
[Environment]: 开篇处于洒满暖调自然光的居家桌面场景，木质桌面搭配身后的置物书架与绿植；随后依次切换至暗光氛围感居家环境、光线柔和的日常客厅、居家办公桌面、临窗的阳光早餐桌面，最终回到暗光氛围感桌面场景，全程营造舒适放松的居家使用氛围。
[Tone & Pacing]: 整体节奏舒缓流畅，遵循开箱→功能展示→多场景适配的产品展示逻辑，层层递进清晰呈现产品核心卖点，观感轻松治愈。
[Camera]: 以产品为核心采用多角度运镜，开篇以固定中景展示开箱动作，随后切换微距特写镜头聚焦风扇的调节旋钮，后续通过全景镜头展示风扇在不同场景中的运行状态，镜头移动平稳顺滑，始终清晰突出产品细节与运行效果。
[Lighting]: 开篇采用暖调柔和的自然光铺洒桌面，营造温馨开箱氛围；暗光场景以风扇自带的紫色氛围灯作为核心视觉光源，搭配环境补光勾勒产品轮廓；日间场景采用通透的自然光均匀照射，清晰展现风扇的纯白机身质感与吹风效果。
[Character]: 仅展示人物手部出镜，20-30岁状态，手部皮肤细腻，指甲修剪整齐带有自然透亮的浅淡光泽，手部无多余装饰，动作轻柔利落，传递开箱好物时的期待与细腻感，无人物面部、全身及穿搭展示。

[Shots]
Shot 1:
  - 时长: 0.0sec - 1.5sec
  - 镜头类型: 开箱
  - 画面: 暖光居家木质桌面场景中，印有风扇产品图案的“Hulrers”品牌纸箱置于画面中央，一双手从画面两侧伸出，手指轻轻捏住纸箱的翻盖将其展开，随后将裹着透明塑封膜的白色桌面风扇从纸箱中平稳取出并放置在桌面上，完成开箱动作。
Shot 2:
  - 时长: 1.5sec - 3.0sec
  - 镜头类型: 展示产品易用性
  - 画面: 镜头切换为微距特写视角，聚焦白色风扇机身侧面的调节旋钮，一只手的指尖伸出，轻轻捏住旋钮缓缓转动，动作轻柔精准。
Shot 3:
  - 时长: 3.0sec - 5.5sec
  - 镜头类型: 产品使用场景
  - 画面: 场景切换至暗光居家环境，风扇在桌面上平稳运行，扇圈与底部透出柔和的紫色氛围灯光，光线在深色空间中勾勒出风扇的圆润轮廓，机身中心的品牌标识清晰可见。
Shot 4:
  - 时长: 5.5sec - 7.5sec
  - 镜头类型: 产品有效性
  - 画面: 场景切换至洒满自然光的日常客厅桌面，白色风扇正对镜头运行，强劲的气流将放置在风扇旁桌面上的纸巾吹得不断飘动，画面清晰展现风扇的风力效果，桌面两侧摆放着白色马克杯，背景是装饰有画作的墙面与茂盛绿植。
Shot 5:
  - 时长: 7.5sec - 8.5sec
  - 镜头类型: 产品设计亮点
  - 画面: 场景切换至居家办公桌面，桌面上摆放着笔记本电脑与无线鼠标，一只手握住风扇顶部隐藏式的提手，轻松将整机从桌面上提起，展示产品便于提携的便携设计。
Shot 6:
  - 时长: 8.5sec - 10.0sec
  - 镜头类型: 设置使用场景
  - 画面: 场景切换至临窗的阳光早餐桌面，桌面铺着橙白格纹餐布，摆放着咖啡杯、盛有可颂与葡萄的餐盘，白色风扇放置在餐布上平稳运行，窗外的自然绿植与光线营造出惬意的早餐氛围。
Shot 7:
  - 时长: 10.0sec - 11.5sec
  - 镜头类型: 场景化生活秀
  - 画面: 回到暗光居家桌面场景，风扇持续平稳运行，紫色氛围灯与桌面倒影交相呼应，一旁的笔记本电脑处于开启状态，画面收尾定格在风扇的完整产品形象上。
[Background Sound]:
- 没有旋律性背景音乐，以一系列操作与环境音效构成：包含开箱撕拉声、物品放置的轻响、电子设备启动提示音、旋钮转动的咔哒声、风扇马达启动运转的持续低频嗡鸣声，整体音轨干净清晰，突出产品的操作反馈与运行质感，听感充满科技感与开箱操作感。
[Transition / Editing]:
- 采用流畅的场景转场剪辑，通过自然的场景切换衔接不同的使用环境，整体转场顺滑无卡顿，完整串联起产品开箱、功能演示、多场景适配的全流程展示。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [my-household-955797](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/my-household-955797?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=my-household-955797)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[my-household-955797](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/my-household-955797?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=my-household-955797)
🤖 **Driving an AI agent?** `clipcat prompt search --query "Household Appliances lifestyle scene"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
