---
id: ph-automotive-449045
title: "2 Pcs T10 LED Bulb White W5W T10 Wedge  Interior Car Bulbs Rep — Real Review (Philippines · Automotive & Motorcycle)"
market: ph
category: automotive-motorcycle
video_type: real-review
duration_sec: 36
hook: pov-scenario
hooks: [pov-scenario]
presenter: hands-only
promo: [none]
voiceover: true
voiceover_lang: en
rank: weekly-1
rank_period: 2026-08-31
source: https://www.tiktok.com/share/video/7651383000428449045
creator: "@ysraelgabriel"
published: 2026-06-14
---
# 2 Pcs T10 LED Bulb White W5W T10 Wedge  Interior Car Bulbs Rep — Real Review (Philippines · Automotive & Motorcycle)
**Weekly #1** · 🇵🇭 Philippines · Automotive & Motorcycle · week of 2026-08-31

Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7651383000428449045) by [@ysraelgabriel](https://www.tiktok.com/@ysraelgabriel). Format: **Real Review** · 36s · hook: **POV Scenario** · Hands only · No promo
**Opening line (from the video):** “Guys, let's change to an LED light”

**Pain point it attacks:** Old halogen bulb has insufficient brightness

**Key visuals:** Motorcycle · Halogen Bulb · LED Bulb · Screwdriver · Transparent Plastic Bag · License Plate · Workshop Floor · Shoes

## Prompt (English)
```text
[Style]: Realistic DIY modification tutorial style, with authentic and delicate picture quality. It focuses on fully restoring the entire modification process of the motorcycle LED license plate light and the final lighting effect, highlighting the practicality of the tutorial and the advantages of the product.
[Environment]: Inside a well-lit private motorcycle repair workshop, with other motorcycles, tool storage boxes and brown fabric curtains placed in sequence in the background. The floor is a non-slip matte floor. The license plate position is pre-covered with white paper towels fixed with adhesive tape, creating a real home repair space for motorcycle enthusiasts to do daily modifications.
[Tone & Pacing]: An authentic, down-to-earth motorcycle modification sharing style with a moderately tight and relaxed rhythm. The demonstration of operation steps is clear and gentle, and the lighting effect display segment is smooth and dynamic. The entire video maintains the rigorous yet relaxed sharing atmosphere of a tutorial video.
[Camera]: The whole video is dominated by close-up shots, focusing on the operation details at the installation position of the motorcycle rear license plate light. It is occasionally accompanied by slight natural handheld shaking, and some frames are slightly moved to adjust the perspective, fully capturing every movement detail of the disassembly and assembly operations.
[Lighting]: Based on the daily indoor workshop lighting, it uses the warm yellow light and white light emitted by the original car light and the new LED light during the modification process, as well as the original colored ambient light of the motorcycle as dynamic light sources in the frame. The contrast between light and dark is clear, accurately showing the brightness difference of different lights.
[Character]: A Southeast Asian male aged 20-30, with naturally rough hand skin from long-term contact with machinery. Only his flexible and powerful hands are shown, no makeup, with strong and firm arm lines. His hand movements are steady and accurate, with strong hands-on ability, focused and meticulous, and he is an experienced motorcycle modification enthusiast.

[Shots]
Shot 1:
  - Duration: 0.0sec - 5.5sec
  - Scene Type: Demonstrate operation method
  - Scene: A close-up shot of the rear part of a motorcycle modified with colored ambient lights in the workshop. The person holds the black lamp base of the license plate light with one hand, and uses a screwdriver with the other hand to gently loosen the fixing screw on the lamp base. After the lamp base is loosened, the person slowly takes out the old halogen bulb emitting warm yellow light from the lamp base, and the lit state of the bulb is clearly visible. The camera maintains a fixed close-up, accurately focusing on the interactive movements between the hands and the lamp base, with no obvious displacement.
  - Subject: That's it, boys. They're going to get a light on the nightclub here on the nightclub.
Shot 2:
  - Duration: 5.5sec - 11.5sec
  - Scene Type: Demonstrate ease of use
  - Scene: The person picks up a gray rag, gently wipes away the residual dust around the lamp base, then aligns a brand new bright white LED bulb with the socket and inserts it for installation. The LED bulb instantly emits bright white light, illuminating the paper towel covering the license plate in front. The camera is aimed at the license plate light installation area throughout the process, steadily capturing the coherent movements of wiping and installing the bulb.
  - Subject: Lailid din nga pala, ilalabi natin mga bossing para matangin. stupid sasabat, baby.  At yung mga bossing gulay-white na
Shot 3:
  - Duration: 11.5sec - 13.5sec
  - Scene Type: Highlight product design features
  - Scene: The person's hand holds a transparent sealed plastic ziplock bag with a white label printed with a QR code attached to it. The bag contains two identical LED license plate lights, and the person points at the bulbs inside the bag with their fingertips to show the details of the product accessories. The camera focuses on the hand and the packaging bag, showing the full appearance of the product at close range.
  - Subject: We're going to buy soya.  Two pieces.
Shot 4:
  - Duration: 13.5sec - 24.0sec
  - Scene Type: Demonstrate ease of use
  - Scene: The person picks up the screwdriver again, aligns the lamp base with the installed LED light back to the slot for resetting, tightens all the fixing screws, then unscrews the lamp base again to confirm the powered-on lit state of the bulb, and the warm yellow light illuminates the license plate area. The camera maintains a close-up perspective, following the hand movements to focus on the operation details of screw tightening and lamp base resetting.
  - Subject: Thank you. guys,  yung mga signal light,  mga bossing,  pero dahil parehas naman "I'm not going to be here, but I'm not going to be here, but I'm not going to be here." .
Shot 5:
  - Duration: 24.0sec - 28.5sec
  - Scene Type: Showcase product material
  - Scene: The person displays the replaced old halogen bulb on their fingertips, then turns their palm upward and spreads it out, placing two brand new LED bulbs in the palm for a comparative display, clearly presenting the appearance and size difference between the two types of bulbs. The camera shoots at the palm at close range, slightly adjusting the focus to make the bulb details clearly visible.
  - Subject: The wind is still in the middle of the wind
Shot 6:
  - Duration: 28.5sec - 33.0sec
  - Scene Type: Demonstrate product effectiveness
  - Scene: The workshop environment switches to a low-light environment. The LED license plate light at the rear of the motorcycle emits strong white light, completely illuminating the white paper towel on the front license plate. The camera turns to the rear side of the motorcycle, and the light from the LED light directly projects onto the workshop floor, forming a bright illuminated area. Multiple pairs of shoes placed on the floor and another motorcycle can also be seen in the background. The camera slowly pans from the rear of the motorcycle to the side of the body, fully showing the illumination range of the light on the ground.
  - Subject: But of course, it's a little bit of a secret. Hey, I'm a trip.  I'm a LAD.  I'm a LAD.
Shot 7:
  - Duration: 33.0sec - 36.0sec
  - Scene Type: Show product usage scenario
  - Scene: The person's hand holds the transparent packaging bag containing the LED bulbs again, displaying it against the lit motorcycle license plate light, then the person extends a finger to point at the license plate position of the motorcycle, and finally the frame gradually dims until it turns completely black. The camera is aimed at the hand and the motorcycle license plate area, maintaining a stable close-up, and the frame naturally fades out at the end.
  - Subject: Put it in the middle of the bar
Shot 8:
  - Duration: 36.0sec - 37.0sec
  - Scene Type: Blogger's personal test experience
  - Scene: The frame is completely black, with no other operational actions.
  - Subject: Bye.
[Background Sound]:
- No background music, only pure human voice narration. The content is a humorous, teasing Filipino-language funny sales pitch with a fast speaking pace and tight rhythm, paired with subtle mechanical sounds generated during minor motorcycle operations. The overall ambient sound is clean with no extra noise.
[Transition / Editing]:
- Adopts natural and smooth action-connected editing. Coherent cut transitions are used between each operation step, the transition from disassembly and assembly operations to lighting effect display is smooth, there are no abrupt jump cuts throughout the process, maintaining a coherent viewing experience of the modification tutorial.
```
## 提示词（中文）
```text
[Style]: 写实DIY改装教学风，画面质感真实细腻，着重还原摩托车LED车牌灯的改装全流程与最终灯光效果，突出教程实用性与商品优势。
[Environment]: 光线充足的私人摩托车维修车间内，背景依次摆放其它摩托车、工具收纳盒与棕色布艺窗帘，地面是防滑哑光地坪，牌照位置提前用白色纸巾配合胶带粘贴遮挡，为摩托车爱好者日常改装的真实居家维修空间。
[Tone & Pacing]: 地道接地气的摩托车改装分享风格，节奏松紧适度，操作步骤演示清晰舒缓，灯光效果展示环节流畅带感，全程保持教程类视频严谨又轻松的分享氛围。
[Camera]: 全程以近距离特写镜头为主，聚焦摩托车尾部牌照灯安装位置的操作细节，偶尔配合轻微的手持自然晃动，部分画面略微移动调整视角，完整捕捉每一个拆装操作的动作细节。
[Lighting]: 以车间室内日常照明为基础光线，搭配改装过程中原车灯、新LED灯发出的暖黄光、白光以及摩托原厂的彩色氛围光作为画面动态光源，明暗对比清晰，精准展现不同灯光的亮度差异。
[Character]: 东南亚男性，20-30岁，手部皮肤带有长期接触机械的自然粗糙质感，仅展示灵活有力的双手，无妆容，手臂线条结实有力，手部动作稳准，动手能力强，专注细致，是经验丰富的摩托车改装爱好者。

[Shots]
Shot 1:
  - 时长: 0.0sec - 5.5sec
  - 镜头类型: 演示烹饪方法
  - 画面: 车间内一辆改装了彩色氛围灯的摩托车尾部特写，人物一手扶住牌照灯黑色灯座，另一手使用螺丝刀轻轻松开灯座上的固定螺丝，灯座松动后，人物将发出暖黄色光的旧卤素灯泡从灯座中缓缓取出，灯泡点亮的状态清晰可见。镜头保持固定特写，精准聚焦手部与灯座的互动动作，无明显位移。
  - 口播: That's it, boys. They're going to get a light on the nightclub here on the nightclub.
Shot 2:
  - 时长: 5.5sec - 11.5sec
  - 镜头类型: 展示易用性
  - 画面: 人物拿起一块灰色抹布，轻轻擦拭灯座周围的残留灰尘，随后将一枚全新的亮白色LED灯泡对准卡座插入安装，LED灯泡瞬间发出明亮的白光，照亮前方的牌照遮挡纸巾。镜头全程对准牌照灯安装区域，稳定捕捉擦拭与安装灯泡的连贯动作。
  - 口播: Lailid din nga pala, ilalabi natin mga bossing para matangin. stupid sasabat, baby.  At yung mga bossing gulay-white na
Shot 3:
  - 时长: 11.5sec - 13.5sec
  - 镜头类型: 产品设计亮点
  - 画面: 人物的手举着一个透明密封塑料自封袋，袋子上贴有带二维码的白色标签，袋子里装有两枚同款LED车牌灯，人物用指尖指着袋内的灯泡，展示产品配件细节。镜头聚焦在手部与包装袋上，近距离展示商品全貌。
  - 口播: We're going to buy soya.  Two pieces.
Shot 4:
  - 时长: 13.5sec - 24.0sec
  - 镜头类型: 展示易用性
  - 画面: 人物再次拿起螺丝刀，将安装好LED灯的灯座重新对准卡槽复位，拧紧所有固定螺丝，随后再次拧开灯座，确认灯泡通电点亮的状态，暖黄色的光线照亮牌照区域。镜头保持特写视角，跟随手部动作聚焦螺丝紧固与灯座复位的操作细节。
  - 口播: Thank you. guys,  yung mga signal light,  mga bossing,  pero dahil parehas naman "I'm not going to be here, but I'm not going to be here, but I'm not going to be here." .
Shot 5:
  - 时长: 24.0sec - 28.5sec
  - 镜头类型: 产品材质展示
  - 画面: 人物将更换下来的旧卤素灯泡放在指尖展示，随后掌心朝上摊开，将两枚全新的LED灯泡放置在手掌中进行对比展示，清晰呈现两款灯泡的外观尺寸差异。镜头近距离对着手掌拍摄，轻微调整对焦，让灯泡细节清晰呈现。
  - 口播: The wind is still in the middle of the wind
Shot 6:
  - 时长: 28.5sec - 33.0sec
  - 镜头类型: 产品有效性
  - 画面: 车间环境切换为暗光环境，摩托车尾部的LED车牌灯发出强烈的白光，将前方牌照上的白色纸巾完全照亮，镜头转向摩托车侧后方，LED灯的光线直接投射到车间地面上，形成一片明亮的照明区域，背景里还能看到地面摆放的多双鞋子与另一辆摩托车。镜头从摩托车尾部缓缓平移转向车身侧面，完整展示灯光在地面的照射范围。
  - 口播: But of course, it's a little bit of a secret. Hey, I'm a trip.  I'm a LAD.  I'm a LAD.
Shot 7:
  - 时长: 33.0sec - 36.0sec
  - 镜头类型: 产品使用场景
  - 画面: 人物的手再次举着装有LED灯泡的透明包装袋，对着点亮的摩托车车牌灯进行展示，随后人物伸出手指向摩托车的牌照位置，最后画面逐渐变暗直至变黑。镜头对准手部与摩托车牌照区域，保持稳定特写，画面最终自然淡出。
  - 口播: Put it in the middle of the bar
Shot 8:
  - 时长: 36.0sec - 37.0sec
  - 镜头类型: 博主亲测体验
  - 画面: 画面完全变黑，无其余操作动作。
  - 口播: Bye.
[Background Sound]:
- 无背景音乐，为纯人声旁白，内容是带有幽默调侃语气的菲律宾语搞笑带货段子，语速较快节奏紧凑，搭配少量摩托车操作时产生的细微机械声响，整体环境音干净无多余杂音。
[Transition / Editing]:
- 采用自然流畅的动作衔接剪辑，每一个操作步骤之间使用连贯的切镜转场，从拆装操作到灯光效果展示过渡平滑，全程无生硬跳切，保持改装教程的连贯观感。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [ph-automotive-449045](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/ph-automotive-449045?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=ph-automotive-449045)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[ph-automotive-449045](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/ph-automotive-449045?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=ph-automotive-449045)
🤖 **Driving an AI agent?** `clipcat prompt search "Automotive & Motorcycle"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
