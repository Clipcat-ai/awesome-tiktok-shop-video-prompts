---
id: ph-womenswear-218645
title: "COD 12PCS 12-Piece Japanese-Style… — Handheld Demo (Philippines · Womenswear & Underwear)"
market: ph
category: womenswear-underwear
video_type: handheld-demo
duration_sec: 16
hook: result-first
hooks: [result-first]
presenter: hands-only
promo: [none]
voiceover: true
voiceover_lang: en
rank: weekly-1
rank_period: 2026-08-17
source: https://www.tiktok.com/share/video/7668316860412218645
creator: "@sweetellena"
published: 2026-07-30
---
# COD 12PCS 12-Piece Japanese-Style… — Handheld Demo (Philippines · Womenswear & Underwear)
**Weekly #1** · 🇵🇭 Philippines · Womenswear & Underwear · week of 2026-08-17
Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7668316860412218645) by [@sweetellena](https://www.tiktok.com/@sweetellena). Format: **Handheld Demo** · 16s · hook: **Result First** · Hands only · No promo
**Opening line (quoted from the video):** “12 pcs Breathable Underwear”

**Key visuals:** Woven Mat · Underwear · White Curtain · Vase · Book · Wall Painting · Hand

## Prompt (English)
```text
[Style]: Warm and cozy home goods showcase style, with soft and delicate color tones, warm translucent texture in the highlight areas, and a relaxed and comfortable overall atmosphere.
[Environment]: A well-lit corner of a bedroom during the daytime, with semi-transparent white sheer curtains hanging by the window, soft natural light diffusing into the room. Different colored pairs of underwear are placed on a light pink woven round floor mat, a low side table next to it holds books, a striped vase and cherry blossom branches, and a decorative print with artistic text leans against the wall, creating a clean and elegant setup.
[Tone & Pacing]: A gentle yet rhythmic showcase pace, presenting different details of the underwear in sequence matching the drum beats of the background music. The scene transitions are smooth and natural, each showcase segment clearly highlights the product advantages, with a well-balanced rhythm that alternates between relaxation and emphasis.
[Camera]: Primarily a first-person overhead perspective with smooth and steady camera movement: start with a fixed shot to show the full product, then push in to a macro close-up of the fabric, pull back to a medium shot to show its softness, hold a frame on the waistband details, and finally follow the hand movements to capture the stretching demonstration of its elasticity.
[Lighting]: Soft diffused natural light from the window is used as the main light source, evenly illuminating the entire scene without harsh shadows. It gives the underwear fabric a natural, soft matte sheen, clearly shows the fine texture of the fabric, and enhances the warm home atmosphere.
[Character]: East Asian woman aged 20-30, only her hands and forearms are visible. Her hands have fair, delicate skin, with nails featuring a nude pink gradient base decorated with white bow nail art. Her forearm lines are slender, she wears a creamy yellow loose knit sweater with ribbed cuffs, she has a gentle and delicate temperament, and her hand movements are soft and relaxed.

[Shots]
Shot 1:
  - Duration: 0.0sec - 1.5sec
  - Scene Type: Handheld product showcase
  - Scene: In a warm home setting, a pair of hands in a creamy yellow sweater steadily hold the left and right sides of the waistband of a green pair of underwear, fully presenting the front of the underwear to the camera. Partial outlines of three other pairs of blue, yellow and pink underwear are clearly visible on the floor mat, and white text '12 pcs Breathable Underwear' appears at the top of the frame. The camera stays in a steady fixed position, focused on the unfolded pair of underwear in the hands with no unnecessary movement. Only the hands with bow nail art and the forearms with creamy yellow ribbed cuffs are visible.
  - Subject: Yeah, yeah, but you're your leader, JB Jr.
Shot 2:
  - Duration: 1.5sec - 3.5sec
  - Scene Type: Product material showcase
  - Scene: The camera pushes in to switch to a macro close-up of the green underwear fabric. Several fingers with bow nail art gently brush over the surface of the underwear, clearly showing the fine knitted texture of the fabric, intuitively conveying its smooth and skin-friendly feel to the audience. The white text at the top of the frame remains displayed. The camera slowly moves forward, with precise focus locked on the contact area between the fingers and the fabric, and the background is naturally blurred. Only part of the fingers with delicate bow nail art are visible.
  - Subject: Yeah, yeah, but you're your leader, JB Jr.
Shot 3:
  - Duration: 3.5sec - 5.5sec
  - Scene Type: Product material showcase
  - Scene: The camera pulls back to return to a medium shot. Both hands lift the two side edges of the green pair of underwear and gently shake it back and forth, using the natural draping movement to intuitively demonstrate that the underwear fabric is soft, lightweight and wrinkle-resistant. The other three pairs of underwear on the floor mat are clearly visible in the background. The camera moves steadily backward, naturally transitioning from the fabric close-up back to a full medium shot of the product. The hands and forearms in the creamy yellow sweater are visible.
  - Subject: See y'all. She's Jimmy Handsome Man, Jin Young, Juicyon.
Shot 4:
  - Duration: 5.5sec - 7.5sec
  - Scene Type: Product design highlight
  - Scene: The camera switches to a close-up of the waistband. A finger gently pinches the wide waistband of the green pair of underwear, clearly presenting the black 'VETO' brand logo printed on the front of the waistband to the camera, and the fine stitching details of the waistband are also shown at the same time. The camera maintains fixed focus, steadily framing the waistband logo area to ensure the text details are clearly visible. Only one finger with bow nail art is visible.
  - Subject: Give me Eleganza, Mark Juicy, y'all. Give me happy birthday.
Shot 5:
  - Duration: 7.5sec - 16.0sec
  - Scene Type: Usability demonstration
  - Scene: The camera pulls back to the initial full shot position. Both hands re-hold the left and right waistbands of the green pair of underwear, first fully present the entire front of the underwear towards the camera, then slowly pull the underwear evenly outwards on both sides, demonstrating the loose and highly elastic properties of the underwear from multiple angles in front of the camera throughout the process. The home decor and the other three pairs of underwear in the background always appear as subtle accents in the frame. The camera stays steady and fixed, clearly recording the entire process of the underwear being stretched following the hand movements with no unnecessary shaking. The hands and forearms in the creamy yellow sweater are visible.
  - Subject: See ya. I'm juicy, y'all. He's giving me lovely crazy boy, but I'm juicy. I'm taking my time
[Background Sound]:
- Upbeat TikTok background music in K-pop hip-hop style, fast tempo, loud volume, with strong beats and vocal samples. Names such as 'JUICE', 'Jungkook', 'Jackson' appear repeatedly in the music, creating a strong idol fan support and fan-oriented atmosphere.
[Transition / Editing]:
- Natural and smooth sequential shot editing is used throughout the video, gradually transitioning from the full product view to detailed close-ups, then returning to showcase the overall features. Every change in camera framing connects seamlessly with no abrupt jumps, maintaining a consistent soft visual tone.
```
## 提示词（中文）
```text
[Style]: 温馨治愈系家居好物展示风，画面色调柔和细腻，高光处呈现出温暖的通透质感，整体氛围松弛舒适。
[Environment]: 白天采光良好的卧室一角，窗边垂挂半透白纱帘，柔和自然光漫入室内；浅粉色编织纹圆地垫上摆放不同颜色内裤，旁侧矮边桌放置书籍、条纹花瓶与樱花枝，墙面靠有印艺术字的装饰画，布置干净雅致。
[Tone & Pacing]: 舒缓又富有律动的展示节奏，配合背景音乐鼓点依次呈现内裤不同细节，画面切换流畅自然，每个展示环节清晰突出产品优势，节奏张弛有度。
[Camera]: 以第一人称俯拍视角为主，运镜平稳流畅：初始固定机位展示产品整体，随后推近到面料微距特写，拉远回中景展示柔软状态，定格腰头细节，最后跟随双手动作拍摄拉扯弹性的画面。
[Lighting]: 以窗边漫射的柔和自然光作为主光源，光线均匀覆盖整个场景，无生硬阴影，让内裤面料呈现自然柔和的哑光光泽，清晰展现面料细腻肌理，烘托温馨居家氛围。
[Character]: 20-30岁东亚女性，仅手部及小臂出镜，手部肌肤细腻白皙，指甲为裸粉色渐变底色点缀白色蝴蝶结美甲，小臂线条纤细，身着奶黄色宽松罗纹袖口针织毛衣，气质温柔细腻，手部动作轻柔舒展。

[Shots]
Shot 1:
  - 时长: 0.0sec - 1.5sec
  - 镜头类型: 手持商品展示
  - 画面: 在布置温馨的居家场景中，一双穿着奶黄色毛衣的手稳稳捏住绿色内裤的左右两侧腰头，将内裤正面完整地展示在镜头前方，地垫上还清晰露出另外三条蓝、黄、粉色内裤的部分轮廓，画面上方同步浮现出白色文字“12 pcs Breathable Underwear”。镜头保持平稳的固定机位，聚焦于手中展开的内裤，没有多余的移动。仅露出带有蝴蝶结美甲的双手与奶黄色罗纹袖口的小臂部分。
  - 口播: Yeah, yeah, but you're your leader, JB Jr.
Shot 2:
  - 时长: 1.5sec - 3.5sec
  - 镜头类型: 产品材质展示
  - 画面: 镜头推近切换为绿色内裤面料的微距特写，带有蝴蝶结美甲的几根手指轻轻抚过内裤的表面，清晰地展示出面料细腻的针织肌理，将其顺滑亲肤的质感直观传递给观众，画面上方的白色文字保持显示。镜头向前缓慢推进，对焦精准锁定在手指与面料接触的区域，背景自然虚化。仅露出带有精致蝴蝶结美甲的部分手指。
  - 口播: Yeah, yeah, but you're your leader, JB Jr.
Shot 3:
  - 时长: 3.5sec - 5.5sec
  - 镜头类型: 产品材质展示
  - 画面: 镜头向后拉远回到中景画面，双手拎起绿色内裤的两侧边缘，轻轻来回晃动内裤，利用自然的垂坠晃动效果，直观展现出内裤面料柔软轻盈、不易起皱的特性，地垫上其余三条内裤作为背景清晰可见。镜头平稳向后拉移，从面料特写自然过渡回完整的产品中景画面。露出穿着奶黄色毛衣的双手与小臂部分。
  - 口播: See y'all. She's Jimmy Handsome Man, Jin Young, Juicyon.
Shot 4:
  - 时长: 5.5sec - 7.5sec
  - 镜头类型: 产品设计亮点
  - 画面: 镜头切换到腰头的近景特写，手指轻轻捏住绿色内裤的宽边腰头处，将印在腰头正面的黑色“VETO”品牌logo清晰地对准镜头展示，腰头的细密车缝线细节也一同呈现出来。镜头保持固定对焦，稳稳定格在腰头logo区域，确保文字细节清晰可见。仅露出带有蝴蝶结美甲的一根手指。
  - 口播: Give me Eleganza, Mark Juicy, y'all. Give me happy birthday.
Shot 5:
  - 时长: 7.5sec - 16.0sec
  - 镜头类型: 展示易用性
  - 画面: 镜头拉回到初始的全景机位，双手重新捏住绿色内裤的左右腰头，先将内裤整体正面朝向镜头完整展示，随后双手缓缓向两侧均匀用力拉扯内裤，全程在镜头前多角度展示内裤宽松且富有弹性的特性，背景中的居家布置与其余三条内裤始终作为点缀呈现在画面中。镜头始终保持固定平稳状态，跟随双手的动作清晰记录内裤被拉扯舒展的全过程，没有多余的晃动。露出穿着奶黄色毛衣的双手与小臂部分。
  - 口播: See ya. I'm juicy, y'all. He's giving me lovely crazy boy, but I'm juicy. I'm taking my time
[Background Sound]:
- 非常欢快的TikTok背景音乐，韩流嘻哈风格、快节奏、音量高亢、带有强烈节拍和人声采样，音乐中反复出现“JUICE”“Jungkook”“Jackson”等名字，具有强烈偶像应援和粉丝向氛围。
[Transition / Editing]:
- 全程采用自然流畅的镜头顺接剪辑方式，从整体展示逐步过渡到细节特写，再回归整体特性展示，每一次镜头景别的切换都衔接丝滑，无生硬跳转，保持统一柔和的视觉调性。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [ph-womenswear-218645](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/ph-womenswear-218645?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=ph-womenswear-218645)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[ph-womenswear-218645](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/ph-womenswear-218645?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=ph-womenswear-218645)
🤖 **Driving an AI agent?** `clipcat prompt search --query "Womenswear & Underwear handheld demo"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
