---
id: mx-kitchenware-580820
title: "Vaso térmico infantil de acero… — Handheld Demo (Mexico · Kitchenware)"
market: mx
category: kitchenware
video_type: handheld-demo
duration_sec: 19
hook: curiosity
hooks: [curiosity]
presenter: hands-only
promo: [none]
voiceover: true
voiceover_lang: es
rank: weekly-1
rank_period: 2026-08-17
source: https://www.tiktok.com/share/video/7675537286393580820
creator: "@soymamaysoymaestra"
published: 2026-08-19
---
# Vaso térmico infantil de acero… — Handheld Demo (Mexico · Kitchenware)
**Weekly #1** · 🇲🇽 Mexico · Kitchenware · week of 2026-08-17
Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7675537286393580820) by [@soymamaysoymaestra](https://www.tiktok.com/@soymamaysoymaestra). Format: **Handheld Demo** · 19s · hook: **Curiosity Gap** · Hands only · No promo
**Opening line (translated from the video):** “Are you looking for a thermos for this back to school?”

**Pain point it attacks:** Can't find a suitable thermos for kids back to school

**Key visuals:** Daisy Tablecloth · Thermos Box · Purple Thermos · Pink Thermos · Stainless Steel Liner · Thermos Lid · Hand

## Prompt (English)
```text
[Style]: TikTok-style realistic product review aesthetic, with bright, saturated screen colors that emphasize the soft, delicate pastel texture of the macaron-colored thermos cups, bringing an authentic, real-life filming feel that makes product displays intuitive and clear.
[Environment]: A home indoor desktop scene, covered with a tablecloth that has a white base and light gray daisy patterns. On the warm orange wall in the background, the brand outer packaging boxes of the two thermos cups are placed. The overall lighting is bright and clean, creating a warm and cozy atmosphere for product sharing.
[Tone & Pacing]: The rhythm is relaxed and gentle, with a strong explanatory sharing vibe. Sufficient screen dwell time is reserved for every product function demonstration, allowing viewers to clearly observe core details of the thermos cups such as leak-proof performance and the stainless steel inner liner.
[Camera]: The entire video is shot from a first-person top-down fixed camera position, with the lens focused on the two thermos cups on the table. There are no large-scale camera movements, only extremely slight natural shakes that follow hand movements, to the greatest extent restoring the real perspective of daily unboxing and display.
[Lighting]: Even, bright indoor top-lit diffused natural light, with no harsh shadows. The soft light spreads evenly across the entire table, perfectly presenting the fine matte texture on the surface of the thermos cups and the metallic luster of the inner stainless steel liner.
[Character]: Only a pair of hands appears on screen, no full human figure is shown. The hands have delicate, fair and smooth skin with no obvious fine lines, and the movements are gentle and neat, conveying a careful and patient product sharing vibe, flexibly completing operations such as pouring water, twisting the lid, and flipping the thermos cups.

[Shots]
Shot 1:
  - Duration: 0.0sec - 3.5sec
  - Scene Type: Problem setup
  - Scene: From the top-down perspective, a light purple thermos cup with a purple inner liner and a pink thermos cup are placed on the table, with two orange-and-white outer packaging boxes printed with thermos cup patterns standing in the background. One hand holds the pink lid of the pink thermos cup and slowly pours water into the pink thermos cup, the water level gradually rises, and fine foam forms inside the cup. The title text 'Vaso térmico para niños' decorated with cursive hearts and the white text 'Anti derrames' pop up on the screen in sequence.
  - Subject: ¿Buscas un vaso térmico para este regreso a clases?
Shot 2:
  - Duration: 3.5sec - 7.5sec
  - Scene Type: Product material display
  - Scene: The white text 'Acero inoxidable' pops up in the center of the screen. The light purple thermos cup under the lens is placed with its opening facing upward. One hand holds the pink water cup and pours water into the light purple thermos cup, clear water flows into the cup, layers of fine bubbles form inside, fully demonstrating the stainless steel inner liner structure of the thermos cup.
  - Subject: Yo encontré este y me encantó porque es antiderrapalmático.
Shot 3:
  - Duration: 7.5sec - 11.0sec
  - Scene Type: Ease of use demonstration
  - Scene: After pouring water is finished, one hand pinches the white lid of the light purple thermos cup which has a purple handle, and slowly twists the lid tight along the threads with a steady, smooth motion to ensure the lid is fully tightened and sealed.
  - Subject: y es perfecto para traerlo en la mochila
Shot 4:
  - Duration: 11.0sec - 15.0sec
  - Scene Type: Product effectiveness demonstration
  - Scene: After completing the sealing operation of the light purple thermos cup, the hand moves next to the pink thermos cup, pinches the pink lid of the pink thermos cup which has an orange handle, and twists it tight and sealed along the threads as well. Then both hands pick up the light purple thermos cup and the pink thermos cup respectively, flip both cups completely upside down with their openings facing fully downward, showing the camera that not a single drop of water leaks out from the openings, intuitively demonstrating the leak-proof effect.
  - Subject: o lonchera escolar. Además, hay muchos conocimientos
Shot 5:
  - Duration: 15.0sec - 19.5sec
  - Scene Type: Display of multiple style options
  - Scene: The hands place the two thermos cups back upright, with their lids with handles facing upward and stably set down on the daisy-patterned tablecloth. The two macaron-colored thermos cups are fully presented in the frame, with the product packaging boxes in the background also clearly visible. The screen freezes to showcase the overall appearance and shape of the products.
  - Subject: colores. Gracias.
[Background Sound]:
- A very cheerful TikTok-style background music with a brisk, lively rhythm, moderately high volume, and electronic pop elements. The melody has strong memorability and a distinct beat, perfectly suited for campus life and student product marketing videos, creating an atmosphere of youth, energy, practicality and fashion. The song style is similar to 'Pop Dance' or 'Gen Z Trendy', and pairs well with keywords such as 'Back to School Season', 'Must-Have for Students', 'Portable Design' to boost audience immersion and purchasing impulse.
[Transition / Editing]:
- Natural, coherent action-matched editing is used, with every operation segment seamlessly connected without abrupt jumps. The screen perspective remains consistent throughout, allowing viewers to smoothly follow the demonstration rhythm to watch the full function display of the thermos cups.
```
## 提示词（中文）
```text
[Style]: TikTok写实好物测评风，画面色彩明亮饱和，着重突出保温杯马卡龙色系的粉嫩质感，自带生活化的真实拍摄感，产品展示直观清晰。
[Environment]: 居家室内桌面场景，铺有白色底带浅灰色雏菊花纹的桌布，背景的暖橙色墙面上摆放着两款保温杯的品牌外包装盒，整体光线充足干净，营造出温馨舒适的好物分享氛围。
[Tone & Pacing]: 节奏轻松平缓，讲解式分享感十足，每一步产品功能演示都留足画面停留时间，让观众可以清晰观察到保温杯的防漏、不锈钢内胆等核心细节。
[Camera]: 全程采用第一人称俯视固定机位拍摄，镜头聚焦桌面的两款保温杯，全程没有大幅度运镜，偶尔随手部动作有极其轻微的自然晃动，最大程度还原日常开箱展示的真实视角。
[Lighting]: 均匀明亮的室内顶光漫射自然光，没有生硬的阴影，光线柔和地铺满整个桌面，完美呈现出保温杯表面细腻的哑光质感与内部不锈钢内胆的金属光泽。
[Character]: 仅出镜一双手，无完整人物形象展示，手部皮肤状态细腻，肌肤白皙光滑无明显细纹，动作轻柔利落，透着细致耐心的好物分享感，灵活完成倒水、旋盖、翻转保温杯等操作。

[Shots]
Shot 1:
  - 时长: 0.0sec - 3.5sec
  - 镜头类型: 设置问题
  - 画面: 俯视视角下，桌面摆放着紫色内胆的浅紫色保温杯与粉色保温杯，背景立着两个带有保温杯图案的橙白配色外包装盒。一只手拿着粉色保温杯的粉色杯盖往粉色保温杯里缓缓倒水，水面逐渐升高，杯内泛起细密泡沫，画面依次弹出花体爱心装饰的标题文字“Vaso térmico para niños”和白色文字“Anti derrames”。
  - 口播: ¿Buscas un vaso térmico para este regreso a clases?
Shot 2:
  - 时长: 3.5sec - 7.5sec
  - 镜头类型: 产品材质展示
  - 画面: 画面中央弹出白色文字“Acero inoxidable”，镜头下的浅紫色保温杯敞口朝上摆放，一只手拿着粉色水杯往浅紫色保温杯中倒水，透明的水流注入杯内，杯内泛起层层细密的水泡，完整展示保温杯的不锈钢内胆结构。
  - 口播: Yo encontré este y me encantó porque es antiderrapalmático.
Shot 3:
  - 时长: 7.5sec - 11.0sec
  - 镜头类型: 展示易用性
  - 画面: 倒水完成后，一只手捏住浅紫色保温杯带有紫色提手的白色杯盖，顺着螺纹慢慢旋紧杯盖，动作平稳顺滑，确保杯盖完全拧紧密封。
  - 口播: y es perfecto para traerlo en la mochila
Shot 4:
  - 时长: 11.0sec - 15.0sec
  - 镜头类型: 产品有效性
  - 画面: 完成浅紫色保温杯的密封操作后，手部移到粉色保温杯旁，捏住粉色保温杯的带橙色提手的粉色杯盖，同样顺着螺纹旋紧密封，随后两只手分别拿起浅紫色保温杯与粉色保温杯，将两个保温杯整体倒置，杯口完全朝下，向镜头展示杯口没有一滴水流出，直观演示防漏效果。
  - 口播: o lonchera escolar. Además, hay muchos conocimientos
Shot 5:
  - 时长: 15.0sec - 19.5sec
  - 镜头类型: 展示多款式选择
  - 画面: 手部将两款保温杯重新摆正，带提手的杯盖朝上平稳放置在带雏菊花纹的桌布上，两款马卡龙色系的保温杯完整地呈现在画面中，背景的产品包装盒也清晰可见，画面定格展示产品的整体外观造型。
  - 口播: colores. Gracias.
[Background Sound]:
- 非常欢快的TikTok风格背景音乐，节奏轻快活泼、音量适中偏高、带有电子流行元素，旋律具有强烈记忆点和节奏感，适合校园生活、学生用品类带货视频，营造青春、活力、实用又时尚的氛围。歌曲风格类似“Pop Dance”或“Gen Z Trendy”，适合搭配“开学季”“学生党必备”“便携设计”等关键词，增强观众代入感与购买冲动。
[Transition / Editing]:
- 采用自然连贯的动作顺接剪辑，每一个操作环节无缝衔接，无生硬跳转，全程保持画面视角统一，让观众可以流畅地跟随演示节奏看完保温杯的全部功能展示。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [mx-kitchenware-580820](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/mx-kitchenware-580820?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=mx-kitchenware-580820)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[mx-kitchenware-580820](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/mx-kitchenware-580820?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=mx-kitchenware-580820)
🤖 **Driving an AI agent?** `clipcat prompt search --query "Kitchenware handheld demo"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
