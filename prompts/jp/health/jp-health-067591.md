---
id: jp-health-067591
title: "Ankle Band（アンクルバンド）半透明 — Real Review (Japan · Health)"
market: jp
category: health
video_type: real-review
duration_sec: 28
hook: pov-scenario
hooks: [pov-scenario]
presenter: hands-only
promo: [none]
voiceover: true
voiceover_lang: ja
rank: weekly-1
rank_period: 2026-08-31
source: https://www.tiktok.com/share/video/7677218654836067591
creator: "@oyabin315"
published: 2026-08-23
---
# Ankle Band（アンクルバンド）半透明 — Real Review (Japan · Health)
**Weekly #1** · 🇯🇵 Japan · Health · week of 2026-08-31

Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7677218654836067591) by [@oyabin315](https://www.tiktok.com/@oyabin315). Format: **Real Review** · 28s · hook: **POV Scenario** · Hands only · No promo
**Opening line (from the video):** “I've used it for 2 months, it's safe when walking, my metabolism improved”

**Key visuals:** Ankle Band · Wooden Floor · Black Pants · Foot Skin · Indoor Light · Japanese Text

## Prompt (English)
```text
[Style]: Realistic lifestyle product review style, focusing on detailed display of foot wearing details, with clean and clear imagery that highlights the wearing experience and effect presentation of the foot ring product.
[Environment]: Bright and minimalist indoor home scene, with a light-colored smooth wall as the background, and light natural wood grain flooring underfoot, no extra clutter to cause distraction. The entire space is neat and fresh with even and soft lighting.
[Tone & Pacing]: Japanese voiceover product sharing style, with a brisk and neat pace. The camera moves naturally following the foot movements, transitions are smooth, the entire video focuses on product wearing and effect demonstration, with no redundant or dragging shots.
[Camera]: The entire video uses a fixed low-angle upward shooting perspective, accurately aimed at the character's foot area. There is no large-scale camera movement throughout the process, only slight panning to follow the feet's side-to-side movement and stepping motions, to maximize the display of the ankle foot ring's wearing state and dynamic foot details.
[Lighting]: Even and soft diffused indoor top lighting, no harsh hard shadows. The light clearly illuminates the foot skin and the translucent material of the foot ring, resulting in a bright and clean image with fully presented details.
[Character]: Asian male, 20-30 years old, with smooth and delicate foot skin of even natural tone. The video only shoots the lower body without showing the face, no makeup, well-proportioned build with smooth leg lines, wearing loose black casual trousers with the cuffs rolled up, with a casual home vibe and natural, relaxed movements.

[Shots]
Shot 1:
  - Duration: 0.0sec - 4.5sec
  - Scene Type: Product usage scene
  - Scene: Low-angle camera aimed at the character's bare feet standing on the wood grain floor. A black foot ring is worn on the right ankle, then the character slowly lifts the left foot and puts the foot ring with a transparent section onto the left ankle. The character alternately lifts each foot and rotates the ankles to show the wearing state of the foot rings, with corresponding Japanese explanatory text overlaid on the screen.
  - Subject: アンクルバンド2ヶ月懸賞生活
正直予想会でした。起きてる時、安心。
Shot 2:
  - Duration: 4.5sec - 11.5sec
  - Scene Type: Product effectiveness
  - Scene: The character continues to alternately lift both feet and walks slowly in place, showing the active state of both feet wearing different colored foot rings from multiple front and side angles. The camera slightly adjusts the frame position following the foot movements, while Japanese text introducing usage scenarios and physical changes is displayed on the screen at the same time.
  - Subject: フルバット装着して生活して私の体重は
新人代謝が良くなり健康的に変化
Shot 3:
  - Duration: 11.5sec - 19.5sec
  - Scene Type: Product effectiveness
  - Scene: The character stands with both feet side by side on the floor, alternately lifting one heel and flipping the ankle backward, clearly showing the full shape of the foot ring at the heel position and its wearing fit. Japanese text explaining the principle of how the product improves physical state is overlaid on the screen.
  - Subject: 感性的腰痛が日に日に和らいで楽に
なり、姿勢もシュット!これはアクロバンドで
Shot 4:
  - Duration: 19.5sec - 27.0sec
  - Scene Type: Ease of use demonstration
  - Scene: The character squats on the floor with one knee bent, picks up a transparent foot ring in hand, slips the foot ring over the top of the foot, then pulls both ends of the foot ring backward to adjust it to the correct wearing position that fits the arch and ankle. The character uses both hands to repeatedly adjust the edges of the foot ring to ensure it is worn smoothly and snugly, while Japanese text explaining the product's mechanism of action is displayed on the screen.
  - Subject: 足の裏のアーチを正しく形成することで、ぜひ、
肩甲斐筋肉が正しく連動して動いてるから
代謝が良くなる体づくりをサポートを
Shot 5:
  - Duration: 27.0sec - 28.5sec
  - Scene Type: Order process guidance
  - Scene: After adjusting the transparent foot ring on his foot, the character extends one hand and points his index finger at the foot ring he is wearing, clearly guiding the visual focus of the product to the foot ring position. Japanese text guiding viewers to check for more details is displayed on the screen.
  - Subject: 気になる人はぜひチェック!
[Background Sound]:
- Upbeat and positive pop electronic background music with a brisk tempo of around 120 BPM, featuring light drum beats and bright synthesizer melodies, at a moderately high volume, creating an atmosphere of healthy living and transformative surprises. The style is well-suited for energetic fitness and lifestyle improvement e-commerce short videos, and its motivating lively rhythm matches the content nature of combining real experience and effect demonstration.
[Transition / Editing]:
- Seamless continuous editing method, naturally switching display angles following foot movements, keeping the visual focus of the frame always on the foot ring product, with a smooth and unified overall rhythm.
```
## 提示词（中文）
```text
[Style]: 生活化好物测评写实风，聚焦脚部穿戴细节展示，画面干净通透，突出足环产品的使用体验与效果呈现。
[Environment]: 明亮简约的居家室内场景，浅色系平整墙面作为背景，脚下铺设浅原木色木纹地板，无多余杂物干扰，整体空间整洁清爽，光线均匀柔和。
[Tone & Pacing]: 日语口播的好物分享风格，节奏轻快利落，镜头跟随脚部动作自然移动，转场丝滑，全程聚焦产品穿戴与效果展示，没有冗余拖沓的镜头。
[Camera]: 全程采用固定低机位仰拍视角，精准对准人物的脚部区域，全程无大幅度机位移动，仅跟随脚部的左右移动、抬步动作进行轻微的画面跟随，最大化突出脚踝足环的穿戴状态与脚部动态细节。
[Lighting]: 均匀柔和的室内顶光散射照明，无刺眼硬阴影，光线清晰照亮脚部皮肤与足环的通透材质，画面明亮干净，细节呈现完整。
[Character]: 亚洲男性，20-30岁，脚部皮肤细腻光滑肤色均匀自然，视频仅拍摄下半身未露面部，无妆容，身形匀称腿部线条流畅，身穿裤脚向上翻折的宽松黑色休闲长裤，居家随性气质，动作自然舒展。

[Shots]
Shot 1:
  - 时长: 0.0sec - 4.5sec
  - 镜头类型: 产品使用场景
  - 画面: 低机位镜头对准人物的双脚，人物赤脚站立在原木色地板上，右脚脚踝佩戴黑色足环，随后人物缓慢抬起左脚，将带有透明部分的足环佩戴在左脚脚踝处，双脚交替抬脚、转动脚踝展示足环的穿戴状态，画面上叠加对应的日文说明文字。
  - 口播: アンクルバンド2ヶ月懸賞生活
正直予想会でした。起きてる時、安心。
Shot 2:
  - 时长: 4.5sec - 11.5sec
  - 镜头类型: 产品有效性
  - 画面: 人物继续交替抬起双脚，缓慢原地行走，从正面、侧面多角度展示双脚佩戴不同颜色足环后的活动状态，镜头跟随脚部移动轻微调整画面位置，画面同步展示介绍使用场景与身体变化的日文文字。
  - 口播: フルバット装着して生活して私の体重は
新人代謝が良くなり健康的に変化
Shot 3:
  - 时长: 11.5sec - 19.5sec
  - 镜头类型: 产品有效性
  - 画面: 人物双脚并排站立在地板上，交替抬起单侧脚后跟，向后翻转脚踝，清晰展示足环在脚后跟位置的完整形态与佩戴贴合度，画面上叠加说明产品改善身体状态原理的日文文字。
  - 口播: 感性的腰痛が日に日に和らいで楽に
なり、姿勢もシュット!これはアクロバンドで
Shot 4:
  - 时长: 19.5sec - 27.0sec
  - 镜头类型: 展示易用性
  - 画面: 人物单脚屈膝蹲坐在地板上，手中拿起一只透明足环，将足环从脚面位置套入，随后向后拉拽足环两端，将足环调整至贴合足弓与脚踝的正确佩戴位置，双手反复调整足环边缘确保穿戴平整服帖，画面同步展示讲解产品作用机制的日文文字。
  - 口播: 足の裏のアーチを正しく形成することで、ぜひ、
肩甲斐筋肉が正しく連動して動いてるから
代謝が良くなる体づくりをサポートを
Shot 5:
  - 时长: 27.0sec - 28.5sec
  - 镜头类型: 下单流程指引
  - 画面: 人物调整完脚上的透明足环后，伸出一只手，用食指指向自己脚上佩戴好的足环，清晰将产品视觉焦点引导至足环位置，画面展示引导观众查看详情的日文文字。
  - 口播: 気になる人はぜひチェック!
[Background Sound]:
- 轻快积极的流行电子背景音乐，节奏明快约120 BPM，带有轻盈的鼓点和明亮的合成器旋律，音量适中偏高，营造出健康生活、蜕变惊喜的氛围，风格适配活力健身与生活改善类电商短视频，自带激励感的轻快节奏贴合真实体验加效果展示的内容属性。
[Transition / Editing]:
- 采用无痕迹连贯剪辑方式，跟随脚部动作自然切换展示角度，保持画面视觉焦点始终集中在足环产品上，整体节奏流畅统一。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [jp-health-067591](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/jp-health-067591?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=jp-health-067591)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[jp-health-067591](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/jp-health-067591?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=jp-health-067591)
🤖 **Driving an AI agent?** `clipcat prompt search "Health"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
