---
id: fr-computers-548704
title: "Grabie marqueurs acryliques noir &… — Real Review (France · Computers & Office Equipment)"
market: fr
category: computers-office-equipment
video_type: real-review
duration_sec: 34
hook: curiosity
hooks: [curiosity]
presenter: hands-only
promo: [none]
voiceover: true
voiceover_lang: es
rank: weekly-1
rank_period: 2026-08-17
source: https://www.tiktok.com/share/video/7674643170239548704
creator: "@cololive6"
published: 2026-08-16
---
# Grabie marqueurs acryliques noir &… — Real Review (France · Computers & Office Equipment)
**Weekly #1** · 🇫🇷 France · Computers & Office Equipment · week of 2026-08-17
Reverse-engineered from [a real TikTok Shop selling video](https://www.tiktok.com/share/video/7674643170239548704) by [@cololive6](https://www.tiktok.com/@cololive6). Format: **Real Review** · 34s · hook: **Curiosity Gap** · Hands only · No promo
**Opening line (translated from the video):** “Wow, what did you notice just now? Let's do this podcast”

**Key visuals:** Cartoon Line Drawing · Black Acrylic Marker · White Acrylic Marker · Marker Set Box · Desk · Manicured Hand · Cream Pattern

## Prompt (English)
```text
[Style]: Realistic product recommendation style, focused on hands-on testing and demonstration of art tools, with sharp, clear image details, vivid and rich color performance, highlighting the smooth coloring texture of the acrylic markers.
[Environment]: A well-lit desktop hand-drawing scene, with a flat tabletop covered in bright, colorful cartoon line art. The desktop is neat and tidy, with only hand-drawn works and the displayed marker products placed on it.
[Tone & Pacing]: A delightful product-sharing style, with a smooth, relaxed visual rhythm. The hand's coloring movements are delicate and continuous, with the entire sequence focused on the hand-drawing creation process and showcasing the product's features.
[Camera]: First-person macro close-up perspective throughout the entire video. The lens follows the hand's drawing movements closely, accurately capturing every detail of the marker tip applying color.
[Lighting]: Even, soft natural desktop light covering the entire scene, with no noticeable shadows, clearly presenting the image colors and the fine texture of the marker's applied color.
[Character]: No full person appears in the frame; only one female hand is visible. The nails have creative manicures in pink, white and orange colors, with a delicate, cute style. The movements are gentle and steady, holding the pen smoothly and naturally, focused on completing the coloring operation.

[Shots]
Shot 1:
  - Duration: 0.0sec - 1.0sec
  - Scene Type: Product usage scene
  - Scene: The lens is aimed at a cartoon hand-drawn line art piece. The eye-catching text '!! PÉPITE !!' is fixed at the top of the frame. The line art depicts a cartoon figure of an elderly woman holding a cake decorated with cream and cherries, with a background made of spliced blue and pink sections.
  - Subject: Muy bien, ¿qué te llamó un instante? Vamos a hacer el podcast.
Shot 2:
  - Duration: 1.0sec - 21.0sec
  - Scene Type: Ease of use demonstration
  - Scene: A hand with delicate manicures holds a black fine-tip acrylic marker, carefully coloring along the white line areas of the cartoon figure's tray, cake outline, and cake edge. The brush strokes are even and smooth, the black pigment spreads fully across the white areas, gradually filling in all the designated black sections.
  - Subject: de la ciudad de Madrid. Gracias. La materia En plus, se étale bien. Le noir est opaque. Gracias.
Shot 3:
  - Duration: 21.0sec - 25.0sec
  - Scene Type: Product effectiveness demonstration
  - Scene: The hand switches the marker to a white fine-tip acrylic marker, applying color evenly to the white cream pattern area on the top layer of the cake. The white pigment smoothly covers the original base color, completely filling the entire cream pattern area.
  - Subject: y de que era un poco. Gracias.
Shot 4:
  - Duration: 25.0sec - 28.5sec
  - Scene Type: Product effectiveness demonstration
  - Scene: The lens slowly pulls back from the close-up of the finished colored cake, fully showing the partial cartoon image after coloring, with the clear, striking coloring effect of the black tray and white cream.
  - Subject: Gracias.
Shot 5:
  - Duration: 28.5sec - 31.5sec
  - Scene Type: Product close-up shot
  - Scene: The lens shifts to one side of the desktop, showing the full set of Grabie brand acrylic markers. The product description text '6 ACRYLIC MARKERS EXTRA FINE TIP' is printed on the packaging box, with multiple markers including black and white neatly arranged inside the set.
  - Subject: Gracias.
Shot 6:
  - Duration: 31.5sec - 34.5sec
  - Scene Type: Product close-up shot
  - Scene: The lens moves down to show two separately placed Grabie acrylic markers, one black and one white, with clear brand and product names printed on the pen bodies. Finally the lens slowly stops, and the frame gradually transitions to a pink background.
  - Subject: Gracias. CC por Antarctica Films Argentina
[Background Sound]:
- Light, lively pop electronic style background music with a bright tempo of around 120 BPM, featuring soft synthesizer melodies and drum beats, at a moderately high volume. It is a popular TikTok product promotion BGM that creates a relaxed, trendy product recommendation demonstration atmosphere, enhancing viewers' perception of the product's ease of use and application effects.
[Transition / Editing]:
- Uses natural lens zooming and scene panning to switch between the coloring process and product display segments, with a seamless, smooth rhythm.
```
## 提示词（中文）
```text
[Style]: 产品种草写实风，主打绘画工具实测展示，画面细节清晰锐利，色彩表现鲜活饱满，突出丙烯马克笔流畅的上色质感。
[Environment]: 光线充足的桌面手绘场景，平整桌面铺设着色彩鲜明的卡通线稿画，桌面整洁干净，仅摆放手绘作品与展示的马克笔产品。
[Tone & Pacing]: 充满惊喜感的好物分享风格，画面节奏舒缓流畅，手部涂色动作细腻连贯，全程聚焦手绘创作过程与产品特性展示。
[Camera]: 全程采用第一人称微距特写视角，镜头紧紧跟随手部作画动作移动，精准捕捉笔尖涂色的每一处细节。
[Lighting]: 均匀柔和的桌面自然光全覆盖，无明显阴影，清晰呈现画面色彩与马克笔上色的细腻质感。
[Character]: 无完整出镜人物，仅可见一只女性手部，指甲做了粉白橙配色的创意美甲，风格精致可爱，动作轻柔稳定，握笔流畅自然，专注完成涂色操作。

[Shots]
Shot 1:
  - 时长: 0.0sec - 1.0sec
  - 镜头类型: 产品使用场景
  - 画面: 镜头对准一张卡通手绘线稿画，画面顶部固定显示“!! PÉPITE !!”的醒目文字，画面中可见一位老奶奶手捧着带奶油樱桃装饰的蛋糕的卡通形象线稿，底色由蓝色与粉色拼接构成。
  - 口播: Muy bien, ¿qué te llamó un instante? Vamos a hacer el podcast.
Shot 2:
  - 时长: 1.0sec - 21.0sec
  - 镜头类型: 展示易用性
  - 画面: 一只做着精致美甲的手握着黑色细头丙烯马克笔，沿着卡通形象中托盘、蛋糕轮廓、蛋糕边缘的白色线条区域细致涂色，笔触均匀流畅，黑色颜料在白色区域充分铺开，逐步填充完成所有黑色指定区域。
  - 口播: de la ciudad de Madrid. Gracias. La materia En plus, se étale bien. Le noir est opaque. Gracias.
Shot 3:
  - 时长: 21.0sec - 25.0sec
  - 镜头类型: 产品有效性
  - 画面: 手部将马克笔更换为白色细头丙烯马克笔，笔尖对准蛋糕上层的白色奶油图案区域均匀涂抹，白色颜料顺滑覆盖原有底色，完整填充好整个奶油图案区域。
  - 口播: y de que era un poco. Gracias.
Shot 4:
  - 时长: 25.0sec - 28.5sec
  - 镜头类型: 产品有效性
  - 画面: 镜头从涂色完成的蛋糕特写处缓慢拉远，完整展示涂色完成后的卡通局部画面，黑色托盘与白色奶油的上色效果清晰醒目。
  - 口播: Gracias.
Shot 5:
  - 时长: 28.5sec - 31.5sec
  - 镜头类型: 产品镜头特写
  - 画面: 镜头移动转向桌面一侧，展示整盒Grabie品牌丙烯马克笔套装，包装盒上印有“6 ACRYLIC MARKERS EXTRA FINE TIP”的产品说明文字，套装内的黑、白等多支马克笔整齐陈列。
  - 口播: Gracias.
Shot 6:
  - 时长: 31.5sec - 34.5sec
  - 镜头类型: 产品镜头特写
  - 画面: 镜头下移展示两支单独摆放的黑色、白色Grabie丙烯马克笔，笔身印有清晰的品牌与产品名称，最后镜头缓缓定格，画面逐渐过渡到粉色背景画面。
  - 口播: Gracias. CC por Antarctica Films Argentina
[Background Sound]:
- 轻快活泼的流行电子风格背景音乐，节奏明快约120 BPM，带有轻盈的合成器旋律和鼓点，音量适中偏高，属于TikTok热门带货类BGM，营造轻松时尚的种草演示氛围，提升观众对产品易用性与使用效果的感知。
[Transition / Editing]:
- 采用自然的镜头推拉与场景平移切换，衔接涂色过程与产品展示环节，节奏顺滑流畅。
```
---
▶️ **Shoot this with your own product** — open it in Clipcat and hit Replicate: [fr-computers-548704](https://clipcat.ai/en/tools/ai-tiktok-videos-prompt/fr-computers-548704?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=fr-computers-548704)
▶️ **用这条提示词拍你自己的产品** — 在 Clipcat 打开并一键复刻：[fr-computers-548704](https://clipcat.ai/zh/tools/ai-tiktok-videos-prompt/fr-computers-548704?utm_source=github&utm_medium=repo&utm_campaign=prompt-library&utm_content=fr-computers-548704)
🤖 **Driving an AI agent?** `clipcat prompt search --query "Computers & Office Equipment real review"` — see [clipcat-skill](https://github.com/Clipcat-ai/clipcat-skill)
